...
class MillingProcessData:
    ...
    def extract_bfc_features(self, bfc_data):
        ...
        for process in self.processes:
            features = extract_features(
                bfc_data.loc[bfc_data.id == process],
                column_sort="time",
                column_id="id",
                kind_to_fc_parameters=kind_to_fc_parameters_5[process],
    ...
    def extract_acc_features(self, acc_data):
        ...
        for process in self.processes:
            features = extract_features(
                acc_data.loc[acc_data.id == process],
                column_sort="time",
                column_id="id",
                kind_to_fc_parameters=kind_to_fc_parameters_5[process],
                disable_progressbar=True,
            )
            # dfs.append(features.to_numpy().astype(np.float32))
            dfs = pd.concat([dfs, features], axis=1)
